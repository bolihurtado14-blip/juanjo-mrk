...
ANSWER_PATH=os.path.join(MODEL_DIR,"aswer.pkl")
...
def load_model():
    if(
        ...
        with open (MODEL_PATH,"rd") as f:
            model = pickle.load(f)
        with open (VECTORIZER_PATH,"rd") as f:
            vectorizer = pickle.load(f)
        with open (ANSWER_PATH,"rd") as f:
            unique_answers = pickle.load(f)
        print("Modelo cargado desde disco")
        return model,vectorizer,unique_answers
    else:
        print("No hay modelo guarddao, sera necesario entrenarlo")
